...
export async function requestPasswordReset(email) {
  ...
  // Always return the same response so registered
  // email addresses cannot be discovered.
  if (!user) {
    return;
  }
...
export async function resetPassword(
  ...
  const resetRecord =
    await userRepository.findPasswordResetToken(
      tokenHash
    );
  ...
  await userRepository.markPasswordResetTokenUsed(
    resetRecord.id
  );
}